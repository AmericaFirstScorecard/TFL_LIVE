(() => {
  ...
  function buildHardwareCard(award) {
    ...
    chips.className = "hardware-card__recipients";
    if (award.recipients?.length) {
      award.recipients.forEach((r) => {
        const chip = document.createElement("div");
        chip.className = "hardware-pill";
        chip.textContent = r.display || r.id;
        chips.appendChild(chip);
      });
    } else {
      const chip = document.createElement("div");
      chip.className = "hardware-pill hardware-pill--ghost";
      chip.textContent = "Unassigned";
      chips.appendChild(chip);
    }

    card.innerHTML = `
      <div class="hardware-card__image hardware-card__image--badge" style="background-image:url('${escapeHtml(
        award.image
      )}')"></div>
